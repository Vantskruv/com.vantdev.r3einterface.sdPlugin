...
const BOX_OPTIONS =
{
    ...
    FIX_REAR_AERO: 7,
    FIX_SUSPENSION: 8,
    REFUEL_SAFE: 9,         //The order from REFUEL_SAFE to REFUEL_CALC is important.
    REFUEL_NORMAL: 10,
    REFUEL_RISKY: 11,
    REFUEL_CALC: 12
};
...
const updateUI = (p1) => {
    ...
    if (action == "com.vantdev.r3sd.toggleboxoptionbutton") {
        var tableItems = document.getElementById("table_toggle_box_option").getElementsByTagName("td");
        for (var i = 0; i < tableItems.length; i++) {
            if (!tableItems[i].classList.contains("no-select") && parseInt(tableItems[i].dataset.index) == p1.box_option) {
                tableItems[i].classList.add("selected");
            }
            else {
                tableItems[i].classList.remove("selected");
            }
        }
    }
        ...
        for (var i = 0; i < tableRequestBoxOptions.length; i++) {
            if (tableRequestBoxOptions[i].classList.contains("no-select")) continue;

            if (p1[tableRequestBoxOptions[i].dataset.index]) tableRequestBoxOptions[i].classList.add("selected");
            else tableRequestBoxOptions[i].classList.remove("selected");
        }

        var tableRequestRefuelOptions = document.getElementById("table_request_box_options_refuel").getElementsByTagName("td");
        for (var i = 0; i < tableRequestRefuelOptions.length; i++) {
            if (!tableRequestRefuelOptions[i].classList.contains("no-select") && parseInt(tableRequestRefuelOptions[i].dataset.index) == p1.refuel_option) {
                tableRequestRefuelOptions[i].classList.add("selected");
            }
            else {
                tableRequestRefuelOptions[i].classList.remove("selected");
            }
        }
    }
}
...
function saveSettings()
{
    if (action =="com.vantdev.r3sd.toggleboxoptionbutton") {
        var tableItems = document.getElementById("table_toggle_box_option").getElementsByTagName("td");
        settings.box_option = 0;
        for (var i = 0; i < tableItems.length; i++) {
            if (tableItems[i].classList.contains("selected")) {
                settings.box_option = parseInt(tableItems[i].dataset.index);
                break;
            }
        }
    }
        ...
        for (var i = 0; i < tableItems.length; i++) {
            if (tableItems[i].classList.contains("selected")) {
                settings[tableItems[i].dataset.index] = true;
            }
            else settings[tableItems[i].dataset.index] = false;
        }

        var tableItems = document.getElementById("table_request_box_options_refuel").getElementsByTagName("td");
        settings.refuel_option = BOX_OPTIONS.DO_NOTHING;
        for (var i = 0; i < tableItems.length; i++) {
            if (tableItems[i].classList.contains("selected")) {
                settings.refuel_option = parseInt(tableItems[i].dataset.index);
                break;
            }
        }
    }
...
function handleSdpiItemChange(e, idx) {
    ...
    if (isList && !e.classList.contains("no-select")) {
        const siv = e.closest('.sdpi-item-value');
        if (!siv.classList.contains('multi-select')) {
            for (let x of sdpiItemChildren) if(e!=x) x.classList.remove('selected');
        }
