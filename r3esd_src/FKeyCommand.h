...
public: 
    ...
    */
private:
    static constexpr std::array<KeyCode, 9> extendedKeys{FKeyCommand::KeyCode::UP, FKeyCommand::KeyCode::DOWN, FKeyCommand::KeyCode::LEFT, FKeyCommand::KeyCode::RIGHT,
                                              FKeyCommand::KeyCode::HOME, FKeyCommand::KeyCode::PAGE_UP, FKeyCommand::KeyCode::PAGEDOWN,
                                              FKeyCommand::KeyCode::DEL, FKeyCommand::KeyCode::END};

    static std::tuple<unsigned short, bool>* keyBeingPressed;// = nullptr;

public:
	
    static void releasePressedKey()
    {
        if (keyBeingPressed)
        {
            try
            {
                release(std::get<0>(*keyBeingPressed), std::get<1>(*keyBeingPressed));
            }
            catch (...)
            {
                delete keyBeingPressed;
                keyBeingPressed = nullptr;
            }
        }
    }
    
    static void SendScanCodeKeyPress(KeyCode keyCode, int holdTimeMillis)
	{
    ...
    static void press(unsigned short scanCode, bool extended)
    {
       ...
       /*
        ...
        input[0].u.ki.dwFlags = eventScanCode;
        input[0].u.ki.dwExtraInfo = GetMessageExtraInfo();
        */

        /*
        inputs->type = INPUT_KEYBOARD;
        inputs->u = new InputUnion;
        inputs->u->ki = new KEYBDINPUT;
        inputs->u->ki->wVk = 0;
        inputs->u->ki->wScan = scanCode;
        inputs->u->ki->dwFlags = eventScanCode;
        inputs->u->ki->dwExtraInfo = GetMessageExtraInfo();
        */

        keyBeingPressed = new std::tuple<unsigned short, bool>(scanCode, extended);
    ...
    static void release(unsigned short scanCode, bool extended)
    {
        ...
        SendInput(ARRAYSIZE(input), input, sizeof(INPUT));
        if(keyBeingPressed) delete keyBeingPressed;
        keyBeingPressed = nullptr;
    }
};
